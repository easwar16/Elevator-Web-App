...
function set_outdoor_switch(floor_no, direct, state) 
{
    ...
    if (outdoor_buttons_state[floor_no][direct] === ON) 
    {
        choose_outdoor_witches(floor_no, direct).css('background', 'yellow')
...
function set_indoor_floor_switch_state(floor_no, elevator_no, state) 
{
    if (state === ON) 
    {
        ...
        ext_choose_indoor_foor_switch(floor_no, elevator_no).css({'background': 'yellow'})
    } 
    else 
    {
...
let direction_display_symbol = new Map([
    ...
    [DIRECTION_DOWN, '￬'],
    [DIRECTION_STILL, ''],
    [DIRECTION_MODE_WAITING_FOR_CHANGING, 'C']
])

function set_indoor_direction_display(elevator_no, direct) 
{
    let res = ''
    if (enable_mode_display && elevators[elevator_no].state.now_direction === DIRECTION_STILL && elevators[elevator_no].state.auto_mode_state === MODE_RUNNING) 
    {
        res = 'A'
    } 
    else if (direct === DIRECTION_MODE_WAITING_FOR_CHANGING && (!enable_mode_display || !enable_DIRECTION_MODE_WAITING_FOR_CHANGING_display)) 
    {
        res = ' '
    } 
    else 
...
{
    return template_of_td_elevator_wall_side.replace(/mark_floor_no/g, floor_no)
}
...
{
    elevator_line_height = cal_floor_height() * floor_nums - $(".elevator-main").height() - $('.elevator-window.ceiling.ground').height()
